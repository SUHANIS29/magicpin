...
# Set provider to gemini, groq, openai, anthropic, deepseek, ollama, or openrouter
LLM_PROVIDER = "groq"
...
class GroqProvider(LLMProvider):
    ...
    def _ensure_valid_model(self):
        """Fetch available models from Groq to avoid 404 model_not_found errors."""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.model and self.model in available_ids:
                    return
                for candidate in ["llama3-70b-8192", "llama-3.1-70b-versatile", "llama-3.1-8b-instant", "mixtral-8x7b-32768"]:
                    if candidate in available_ids:
                        print_info(f"Using available Groq model: {candidate}")
                        self.model = candidate
                        return
                if available_ids:
                    self.model = available_ids[0]
                    print_info(f"Using fallback Groq model: {self.model}")
        except Exception:
            if not self.model:
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        for attempt in range(5):
            try:
                resp = requests.post(url, headers=headers, json=payload, timeout=TIMEOUT_LLM)
                if resp.status_code == 200:
                    data = resp.json()
                    return data["choices"][0]["message"]["content"]
                    ...
                    print_fail(f"Groq API Error ({resp.status_code}): {resp.text}")
                    resp.raise_for_status()
            except requests.exceptions.RequestException as e:
                last_err = e
                time.sleep(2)
                continue
        raise last_err or Exception("Groq request failed after retries")
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _extract_json_object(self, text: str) -> Optional[str]:
        """Return the first balanced {...} block, ignoring anything after it."""
        start = text.find('{')
        if start == -1:
            return None
        depth = 0
    ...
    def _parse_response(self, response: str, action: Dict) -> ScoreResult:
        """Parse LLM JSON response cleanly using balanced brace scanning."""
        cleaned = re.sub(r'```(?:json)?', '', response).strip('` \n\r\t')
        obj_str = self._extract_json_object(cleaned)
        if not obj_str:
            return self._fallback_score(action)
        try:
...
class JudgeSimulator:
    ...
    def _full(self) -> bool:
        ...
        for cid, cust in self.dataset.customers.items():
            self.client.push_context("customer", cid, 1, cust)
        for tid, t in self.dataset.triggers.items():
            self.client.push_context("trigger", tid, 1, t)

        print_info(f"Pushed {len(self.dataset.customers)} customer contexts")
        ...
        for i in range(0, len(tids), 5):
            ...
            for action in actions:
                # Set verbose=True to view score reasons and hints directly
                self._score_and_display(action, verbose=True)
